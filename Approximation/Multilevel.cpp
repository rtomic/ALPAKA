...
Multilevel::Multilevel(MultilevelParams &params)
{
		...
		for(std::size_t level = 0; level < m_nLevels; level++)
		{
			...
			m_radius[level] = params.overlap * std::pow(static_cast<double>(m_nPoints[level]),
...
Multilevel::Multilevel(double const &overlap,
					   ...
					   std::unique_ptr<Kernel> motherKernel)
{
	...
	for(std::size_t level = 0; level < m_nLevels; level++)
	{
		...
		m_radius[level] = overlap * std::pow(static_cast<double>(m_nPoints[level]),
											 -1.0/static_cast<double>(m_dimension));
...
double Multilevel::at(std::vector<double> const &x) const
{
	double value = 0.0;
...
/*
 * 		OLD, for large matrices triplets seem to be slow; implementation above allows
 * 		for faster, parallel build of matrix
 *
	    double kernelValue;
	    SparseMatrix A(m_nPoints[level], m_nPoints[level]);
...
void Multilevel::solve(std::function<double(const std::vector<double> &)> const &targetFunc)
{
	{
...
void Multilevel::solve(std::vector<double> const &targetFunc)
{
...
double Multilevel::atLevel(std::size_t level, std::vector<double> const &x) const
{
	double value = 0.0;
...
void Multilevel::updateResiduals(std::size_t const &level,
